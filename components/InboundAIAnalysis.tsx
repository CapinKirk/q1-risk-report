...
function recalculateProductTotals(attainmentRows: AttainmentRow[]): ProductTotal {
  ...
  return {
    ...
    total_qtd_attainment_pct: qtdTarget > 0 ? Math.round((qtdAcv / qtdTarget) * 100) : (qtdAcv > 0 ? 100 : 0),
...
function filterReportData(reportData: ReportData, products: Product[], regions: Region[]): ReportData {
  ...
  return {
    ...
    grand_total: {
      ...
      total_qtd_attainment_pct: combinedQtdTarget > 0 ? Math.round((combinedQtdAcv / combinedQtdTarget) * 100) : (combinedQtdAcv > 0 ? 100 : 0),
...
function toHTMLExport(markdown: string, generatedAt: string | null): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline';"><title>Inbound Marketing Analysis</title>
