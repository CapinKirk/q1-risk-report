            ...
            {sortedData.map((row, idx) => {
              ...
              return (
                <tr key={`${row.region}-${row.source}-${idx}`}>
                  ...
                  <td className="right att-color" style={{ '--att-color': getAttainmentColor(attPct), fontWeight: 600 } as React.CSSProperties}>{formatPercent(attPct)}</td>
                  <td className="right var-color" style={{ '--var-color': getGapColor(gap), fontWeight: 600 } as React.CSSProperties}>{formatCurrency(gap)}</td>
                  <td className="center">
            ...
            {sortedData.map((row, idx) => {
              ...
              return stages.map((stage, stageIdx) => (
                <tr key={`${row.region}-${category}-${stage.label}-${idx}`}>
                  ...
                  <td className="right att-color" style={{ '--att-color': getAttainmentColor(stage.pct), fontWeight: 600 } as React.CSSProperties}>{formatPercent(stage.pct)}</td>
                  <td className="right var-color" style={{ '--var-color': getGapColor(stage.gap), fontWeight: 600 } as React.CSSProperties}>
